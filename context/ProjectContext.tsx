...
import { Project, TextAsset, MediaAsset, Task, UserProfile, Receipt, ChatMessage, Expense, ExpenseTracking, Campaign } from '../types';
...
interface ProjectContextType {
  ...
  toggleExpensePayment: (expenseId: string, month: number, year: number) => Promise<void>;
...
export const ProjectProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <ProjectContext.Provider value={{
      projects, tasks, currentUser, usersDB, receipts, expenses, expenseTracking, campaigns, notifications, baseSalaries, taskRates, financeSettings, studioLogo, dashboardBanner, dashboardBannerTitle, dashboardBannerSubtitle, loginBackground, loginTitle, loginSubtitle, isSyncing, toast, showToast, messages,
      login, logout,
      ...
      toggleExpensePayment: async (expenseId, month, year) => {
        try {
          const existingIndex = expenseTracking.findIndex(t => t.id === expenseId && t.month === month && t.year === year);
          let nextList = [...expenseTracking];
          
          if (existingIndex > -1) {
            const isPaid = nextList[existingIndex].paid;
            nextList[existingIndex] = { ...nextList[existingIndex], paid: !isPaid, paidAt: !isPaid ? new Date().toISOString() : undefined };
          } else {
            nextList.push({ id: expenseId, month, year, paid: true, paidAt: new Date().toISOString() });
          }
          ...
          showToast(existingIndex > -1 && nextList[existingIndex].paid === false ? "Pago cancelado" : "Gasto marcado como pagado");
          fetchData(true);
        } catch (err) {
