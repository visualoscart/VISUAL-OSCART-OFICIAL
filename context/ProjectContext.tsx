...
export const ProjectProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    try {
      ...
      if (tRes.data) {
        setTasks(tRes.data.map(t => ({
          ...
          completedAt: t.completed_at, contentType: t.content_type
        })));
      }
  ...
  return (
    <ProjectContext.Provider value={{
      ...
      addTask: async (data) => {
        const payload = { id: `task-${Date.now()}`, project_id: data.projectId, collaborator_id: data.collaboratorId, title: data.title, description: data.description, date: data.date, drive_link: data.drive_link || '', status: 'Pendiente', content_type: data.contentType };
      ...
      updateTask: async (id, d) => { 
        ...
        if (d.contentType) { mapped.content_type = d.contentType; delete mapped.contentType; }
