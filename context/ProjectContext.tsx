...
import { Project, TextAsset, MediaAsset, Task, UserProfile, Receipt, ChatMessage } from '../types';
...
export const ProjectProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchData = useCallback(async (silent = false) => {
    ...
    try {
      const [uRes, pRes, tRes, rRes, sRes] = await Promise.all([
      ...
      if (pRes.data) {
        setProjects(pRes.data.map(p => ({ 
          ...
          progress: p.progress,
          logoUrl: p.logo_url,
          brandManualUrl: p.brand_manual_url,
          brief: p.brief,
          hell: p.hell,
          heaven: p.heaven,
          monthlyFee: Number(p.monthly_fee),
          textRepository: p.text_repository || [],
          mediaRepository: p.media_repository || [],
      ...
      if (rRes.data) {
        setReceipts(rRes.data.map(r => ({
          ...
          masterBonus: Number(r.master_bonus),
  ...
  return (
    <ProjectContext.Provider value={{
      projects, tasks, currentUser, usersDB, receipts, notifications, baseSalaries, studioLogo, loginBackground, loginTitle, loginSubtitle, isSyncing, toast, showToast, messages,
      login, logout,
      ...
      addTask: async (data) => {
        const payload = { id: `task-${Date.now()}`, project_id: data.projectId, collaborator_id: data.collaboratorId, title: data.title, description: data.description, date: data.date, drive_link: data.driveLink || '', status: 'Pendiente' };
        await supabase.from('tasks').insert([payload]); fetchData(true);
      },
      ...
      toggleTaskStatus: async (id) => {
        ...
        await supabase.from('tasks').update({ status: isDone ? 'Pendiente' : 'Completada', completed_at: isDone ? null : new Date().toISOString() }).eq('id', id); fetchData(true);
      },
      deleteTask: async (id) => { await supabase.from('tasks').delete().eq('id', id); fetchData(true); },
      deleteUser: async (id) => { await supabase.from('users').delete().eq('id', id); fetchData(true); },
      sendReceiptToUser: async (r) => {
        const payload = { user_id: r.userId, user_name: r.userName, month: r.month, year: Number(r.year), base_salary: Number(r.baseSalary), ninja_bonus: Number(r.ninjaBonus), master_bonus: Number(r.masterBonus), total: Number(r.total), date: r.date, receipt_number: r.receiptNumber };
