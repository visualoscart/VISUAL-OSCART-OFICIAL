...
const AdminDashboard: React.FC = () => {
  const { 
    projects, usersDB, sendReceiptToUser, deleteReceipt, updateBaseSalary, updateTaskRate, baseSalaries, taskRates,
    financeSettings, updateFinanceSettings, expenses, expenseTracking, toggleExpensePayment, addExpense, deleteExpense,
  ...
  const [activeView, setActiveView] = useState<'analytics' | 'payroll' | 'clients' | 'users' | 'settings' | 'rendimiento'>('analytics');
  ...
  const [selectedPeriod, setSelectedPeriod] = useState(() => {
    const now = new Date();
    // If today is >= 20, the "current" period we are working on is the one ending next month
    // But usually people want to see the period they are currently in.
    // If today is Feb 25, we are in the period Feb 20 - Mar 19.
    // If today is Feb 15, we are in the period Jan 20 - Feb 19.
  ...
  const getPeriodRange = (month: number, year: number) => {
    // Periodo "Febrero 2026" (month=1, year=2026) -> 20 Ene a 19 Feb
    const startDate = new Date(year, month - 1, 20, 0, 0, 0);
    const endDate = new Date(year, month, 19, 23, 59, 59);
    return { start: startDate, end: endDate };
  };

  const isDateInPeriod = (dateStr: string, month: number, year: number) => {
  ...
  const financeMetrics = useMemo(() => {
    ...
    // Tracking de pagos por periodo seleccionado
    const periodTracking = expenses.map(e => {
      const tracking = expenseTracking.find(t => t.id === e.id && t.month === selectedPeriod.month && t.year === selectedPeriod.year);
      return { ...e, paid: tracking?.paid || false };
    });

    const paidExpenses = periodTracking.filter(e => e.paid).reduce((acc, e) => acc + (Number(e.amount) || 0), 0);
    const pendingExpenses = periodTracking.filter(e => !e.paid).reduce((acc, e) => acc + (Number(e.amount) || 0), 0);
    ...
    return { 
      ...
      periodTracking, paidExpenses, pendingExpenses 
    };
  }, [projects, baseSalaries, usersDB, financeSettings, expenses, expenseTracking, selectedPeriod]);

  const calculateUserBonuses = (userId: string) => {
    const userTasks = tasks.filter(t => 
      String(t.collaboratorId) === String(userId) &&
      isDateInPeriod(t.date, selectedPeriod.month, selectedPeriod.year)
    );
  ...
  const handleProcessPayment = async (user: any) => {
    ...
    console.log("Performance Metrics for all members:", performanceMetrics.byMember);
    console.log("Found member performance:", memberPerf);
    
    const completedTasks = memberPerf?.completed || 0;
    const tasksTotal = memberPerf?.totalPay || 0;

    console.log(`Processing payment for ${user.firstName}. Tasks: ${completedTasks}, Total: ${tasksTotal}`);

    const receiptData = {
      id: '', userId: String(user.id), userName: `${user.firstName} ${user.lastName}`,
      month: monthNames[selectedPeriod.month], year: selectedPeriod.year,
    ...
    <div className="flex-1 flex flex-col h-full bg-transparent overflow-y-auto scrollbar-hide relative pattern-orbital font-display">
      <header className="px-10 py-6 border-b border-white/5 flex items-center justify-between sticky top-0 z-40 bg-background-dark/40 backdrop-blur-3xl">
        ...
        <nav className="flex bg-black/40 p-1.5 rounded-2xl border border-white/5 gap-1.5 backdrop-blur-md">
          {[ {id:'analytics', icon:'grid_view'}, {id:'rendimiento', icon:'monitoring'}, {id:'clients', icon:'handshake'}, {id:'users', icon:'group'}, {id:'payroll', icon:'payments'}, {id:'settings', icon:'tune'} ].map(tab => (
      ...
      <div className="p-8 sm:p-12 pb-24 relative z-10">
        <div className="max-w-7xl mx-auto space-y-8 sm:space-y-12">
          {(activeView === 'rendimiento' || activeView === 'payroll' || activeView === 'analytics') && (
            <div className="flex items-center justify-between glass-panel p-6 rounded-3xl border border-white/5 shadow-xl animate-in fade-in slide-in-from-top-4 duration-700">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Periodo de Gestión</h4>
                  <p className="text-xs font-bold text-white uppercase italic">Corte: 20 al 20 del mes</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <select 
                  className="bg-black/40 border border-white/5 rounded-xl px-4 py-2 text-xs text-white font-bold outline-none focus:border-primary/40"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="p-8 rounded-[2.5rem] border border-white/5 glass-panel flex flex-col justify-between shadow-2xl">
                  <div className="flex justify-between items-start">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest opacity-60">Tareas Completadas Totales</p>
                ...
                <div className="p-8 rounded-[2.5rem] border border-white/5 glass-panel flex flex-col justify-between shadow-2xl">
                  <div className="flex justify-between items-start">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest opacity-60">Tareas Pendientes Totales</p>
                        ...
                        <tr className="hover:bg-white/[0.02] group transition-colors cursor-pointer" onClick={() => setExpandedMemberId(expandedMemberId === m.id ? null : m.id)}>
                          <td className="px-8 py-6 flex items-center gap-5">
                            ...
                            <div>
                              ...
                              <span className="text-[9px] text-slate-500 font-black uppercase tracking-widest">{m.role}</span>
                            </div>
                          </td>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10">
                    <div className="lg:col-span-4 glass-panel p-8 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] space-y-6 border border-white/5 shadow-xl">
                        ...
                        <div className="pt-6 border-t border-white/5 space-y-4">
                            <h3 className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] mb-4">Balance de Egresos Variables</h3>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-emerald-500/5 border border-emerald-500/10 p-4 rounded-2xl">
                                    <p className="text-[8px] font-black text-emerald-500/60 uppercase tracking-widest mb-1">Ejecutado</p>
                                ...
                                <div className="bg-rose-500/5 border border-rose-500/10 p-4 rounded-2xl">
                                    <p className="text-[8px] font-black text-rose-500/60 uppercase tracking-widest mb-1">Por Pagar</p>
                            ...
                            <div className="bg-black/20 border border-white/5 p-4 rounded-2xl flex justify-between items-center">
                                <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Saldo Total Matriz</p>
                    ...
                    <div className="lg:col-span-8 glass-panel p-8 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] space-y-8 border border-white/5 overflow-hidden shadow-xl">
                        <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mt-1">Sincronización de pagos: {["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"][selectedPeriod.month]} {selectedPeriod.year}</p>
                            </div>
                            <div className="flex items-center gap-4">
                              <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest bg-white/5 px-4 py-1.5 rounded-xl border border-white/5 shadow-inner">Configuración Global</span>
                            </div>
                        </div>
