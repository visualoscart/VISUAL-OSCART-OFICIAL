...
const AdminDashboard: React.FC = () => {
  ...
  // Estado para gestionar los cambios de contraseña de forma segura
  ...
  const handleDeleteReceiptStep = async (id: string) => {
    if (receiptToDeleteId === id) { await deleteReceipt(id); setReceiptToDeleteId(null); } 
    else { setReceiptToDeleteId(id); setTimeout(() => setReceiptToDeleteId(null), 3000); }
  };

  const handleUpdateUserPassword = async (userId: string) => {
    const newPass = tempPasswords[userId];
    if (!newPass) return;
    const ok = await updateUser(userId, { password: newPass });
    if (ok) {
        ...
        <div className="max-w-6xl mx-auto space-y-12">
          ...
          {activeView === 'payroll' && (
             <div className="space-y-12 animate-in">
                    ...
                    <table className="w-full text-left">
                        ...
                        {usersDB.map(u => {
                            ...
                            return (
                                ...
                                <td className="px-10 py-6 font-bold text-sm text-white uppercase tracking-tight">
                                  <div className="flex items-center gap-4">
                                    <img src={u.avatar} className="w-10 h-10 rounded-xl object-cover border border-white/10 shadow-sm" alt="" />
                ...
                <div className="glass-panel rounded-[3rem] overflow-hidden shadow-2xl">
                    <div className="p-10 border-b border-white/5">
                        <h3 className="text-xl font-display text-white uppercase italic tracking-tight">Historial de Liquidaciones</h3>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-white/5 text-[9px] uppercase text-slate-500 font-bold tracking-widest border-b border-white/5">
                            ...
                            <tbody className="divide-y divide-white/5">
                                {receipts.map(r => {
                                    const socio = usersDB.find(usr => usr.id === r.userId);
                                    return (
                                        <tr key={r.id} className="hover:bg-white/[0.01] group transition-colors">
                                            <td className="px-10 py-6 font-mono text-xs text-slate-400">{r.receiptNumber}</td>
                                            <td className="px-10 py-6 text-sm font-bold text-white uppercase">
                                              <div className="flex items-center gap-4">
                                                <img src={socio?.avatar || `https://api.dicebear.com/7.x/initials/svg?seed=${r.userName}`} className="w-8 h-8 rounded-lg object-cover border border-white/5" alt="" />
                                                {r.userName}
                                              </div>
                                            </td>
                                            <td className="px-10 py-6 font-display text-emerald-400 font-bold">${(Number(r.total) || 0).toFixed(0)}</td>
                                            <td className="px-10 py-6 text-right">
                                                <button onClick={() => handleDeleteReceiptStep(r.id)} className={`px-4 py-2 rounded-xl border transition-all text-[9px] font-black uppercase tracking-widest ${receiptToDeleteId === r.id ? 'bg-rose-600 text-white border-rose-400 animate-pulse' : 'bg-rose-500/10 text-rose-500 border-rose-500/20 hover:bg-rose-500 hover:text-white opacity-0 group-hover:opacity-100'}`}>
                                                    <span className="material-symbols-outlined text-sm">{receiptToDeleteId === r.id ? 'warning' : 'delete_forever'}</span>
                                                    {receiptToDeleteId === r.id && "CONFIRMAR"}
                                                </button>
                                            </td>
                                        </tr>
                                    );
                  ...
                  <tbody className="divide-y divide-white/5">
                    {projects.map(p => (
                      <tr key={p.id} className="hover:bg-white/[0.01]">
                        <td className="px-10 py-6 font-bold text-white uppercase text-sm">
                          <div className="flex items-center gap-4">
                            <img src={p.logoUrl || `https://api.dicebear.com/7.x/initials/svg?seed=${p.name}`} className="w-10 h-10 rounded-xl object-cover border border-white/10" alt="" />
                            {p.name}
                          </div>
                        </td>
                        <td className="px-10 py-6">
              ...
              <div className="glass-panel rounded-[3rem] overflow-hidden">
                 <table className="w-full text-left">
                    <thead className="bg-white/5 text-[9px] uppercase text-slate-500 font-bold tracking-widest">
                      <tr>
                        <th className="px-10 py-5">Socio</th>
                        <th className="px-10 py-5">Puesto</th>
                        <th className="px-10 py-5">Gestión de Acceso (Password)</th>
                        <th className="px-10 py-5 text-right">Acción</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                      {usersDB.map(u => (
                        <tr key={u.id} className="hover:bg-white/[0.01]">
                          <td className="px-10 py-6 text-sm font-bold text-white uppercase">
                            <div className="flex items-center gap-4">
                              <img src={u.avatar} className="w-10 h-10 rounded-xl object-cover border border-white/10" alt="" />
                              {u.firstName} {u.lastName}
                            </div>
                          </td>
                          ...
                          <td className="px-10 py-6">
                            <div className="flex items-center gap-2 max-w-[240px]">
                              <input 
                                type="text" 
                                className="flex-1 bg-black/40 border border-white/5 rounded-xl px-4 py-2 text-white font-mono text-xs outline-none focus:border-primary" 
                              ...
                              <button 
                                ...
                                className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all ${
                                  tempPasswords[u.id] !== undefined 
                                  ? 'bg-primary text-white shadow-lg' 
                                  : 'bg-white/5 text-slate-600 hover:text-primary'
                                }`}
               ...
               <div className="glass-panel p-10 rounded-[3rem] space-y-8">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      {/* Fixed: Added 'e => ' to onChange handler to fix "Cannot find name 'e'" error */}
