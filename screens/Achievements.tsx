...
const Achievements: React.FC = () => {
  ...
  // Mismo periodo operativo que en AdminDashboard: 20 del mes anterior al 19 del actual
  // Si hoy es >= 20, estamos en el periodo que cierra el 19 del próximo mes
  // Pero para visualizar el "actual", usamos el que cierra hoy o próximamente.
  ...
  const { start: periodStart, end: periodEnd } = getPeriodRange(currentMonth, currentYear);

  const relevantTasks = tasks.filter(t => {
    if (t.collaboratorId !== currentUser?.id) return false;
    return isDateInPeriod(t.date, currentMonth, currentYear);
  });

  const evaluationSet = relevantTasks;
  ...
  const isStrategyMaster = allTasksDone && evaluationSet.every(t => {
    ...
    const completedDate = new Date(t.completedAt);
    const diffDays = Math.floor((deadline.getTime() - completedDate.getTime()) / (1000 * 3600 * 24));
    return diffDays >= 7;
  });

  const ninjaProgress = evaluationSet.length > 0 
    ? (evaluationSet.filter(t => {
        if (t.status !== 'Completada' || !t.completedAt) return false;
        const deadline = new Date(t.date + 'T23:59:59');
        return new Date(t.completedAt) <= deadline;
      }).length / evaluationSet.length) * 100 
    : 0;

  const masterProgress = evaluationSet.length > 0 
    ? (evaluationSet.filter(t => {
        if (t.status !== 'Completada' || !t.completedAt) return false;
        const deadline = new Date(t.date + 'T12:00:00');
        const diffDays = Math.floor((deadline.getTime() - new Date(t.completedAt).getTime()) / (1000 * 3600 * 24));
        return diffDays >= 7;
      }).length / evaluationSet.length) * 100 
    : 0;
