...
const Achievements: React.FC = () => {
  ...
  const getPeriodRange = (month: number, year: number) => {
    // Periodo que cierra el 19 de 'month'
  ...
  const { start: periodStart, end: periodEnd } = getPeriodRange(reportingMonth, reportingYear);

  const relevantTasks = tasks.filter(t => {
    if (t.collaboratorId !== currentUser?.id) return false;
    return isDateInPeriod(t.date, reportingMonth, reportingYear);
  });
  ...
  const isStrategyMaster = allTasksDone && evaluationSet.every(t => {
    ...
    // Comparación robusta de días: (Fecha Tarea) - (Fecha ejecución)
  ...
  const ninjaProgress = evaluationSet.length > 0 
    ? (evaluationSet.filter(t => {
        ...
        // Tarea a tiempo: se completó el mismo día o antes
        const taskDate = new Date(t.date + 'T12:00:00');
        const completedDate = new Date(t.completedAt);
        const d1 = new Date(taskDate.getFullYear(), taskDate.getMonth(), taskDate.getDate());
        const d2 = new Date(completedDate.getFullYear(), completedDate.getMonth(), completedDate.getDate());
