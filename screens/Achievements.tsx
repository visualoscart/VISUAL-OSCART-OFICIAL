...
const Achievements: React.FC = () => {
  ...
  // Tareas del socio para el mes actual
  ...
  const completedMonthlyTasks = monthlyTasks.filter(t => t.status === 'Completada');

  // REGLA: Todas las tareas deben estar completadas para que el logro se evalúe
  const allTasksDone = monthlyTasks.length > 0 && completedMonthlyTasks.length === monthlyTasks.length;

  const isMarketingNinja = allTasksDone && monthlyTasks.every(t => {
  ...
  const isStrategyMaster = allTasksDone && monthlyTasks.every(t => {
    if (!t.completedAt) return false;
    const deadline = new Date(t.date);
    const completedDate = new Date(t.completedAt);
  ...
  const LOGROS_CONFIG = [
    {
      ...
      icon: 'military_tech',
      description: 'Completa EL 100% de tus tareas del mes a tiempo o antes del vencimiento.',
      bonus: '$10 USD Bono Mensual',
      earned: isMarketingNinja,
      color: 'from-blue-500 to-indigo-600',
      shadow: 'shadow-blue-500/20',
      progress: monthlyTasks.length > 0 ? (completedMonthlyTasks.length / monthlyTasks.length) * 100 : 0,
      requirement: 'Perfección Puntual (100%)'
    },
    {
      id: 'master',
      title: 'Strategy Master',
      icon: 'verified',
      description: 'Completa EL 100% de tus tareas del mes con al menos una semana (7 días) de anticipación.',
      bonus: '$20 USD Bono Mensual',
      earned: isStrategyMaster,
      color: 'from-amber-400 to-orange-600',
      shadow: 'shadow-amber-500/20',
      progress: monthlyTasks.length > 0 ? (monthlyTasks.filter(t => {
        if (!t.completedAt) return false;
        const diffDays = Math.floor((new Date(t.date).getTime() - new Date(t.completedAt).getTime()) / (1000 * 60 * 60 * 24));
        return diffDays >= 7;
      }).length / monthlyTasks.length) * 100 : 0,
      requirement: 'Maestría Anticipada (100%)'
    }
  ];
    ...
    <div className="flex-1 flex flex-col h-full bg-background-dark overflow-hidden font-display">
      <header className="px-8 py-8 border-b border-white/5 shrink-0 bg-background-dark/50 backdrop-blur">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Rendimiento: {now.toLocaleString('es-ES', { month: 'long', year: 'numeric' })}</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="px-6 py-3 bg-white/5 rounded-2xl border border-white/10 text-center">
               <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Tareas Mensuales</p>
               <p className="text-lg font-black text-white">{completedMonthlyTasks.length} / {monthlyTasks.length}</p>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 scroll-smooth scrollbar-hide">
        <div className="max-w-7xl mx-auto space-y-12 pb-20">
          {!allTasksDone && monthlyTasks.length > 0 && (
            <div className="bg-amber-500/10 border border-amber-500/20 p-6 rounded-[2rem] flex items-center gap-4 text-amber-500">
               ...
               <div>
                  <p className="text-sm font-black uppercase tracking-widest">Estado: Evaluando Periodo</p>
                  <p className="text-xs opacity-80">Debes completar TODAS las tareas asignadas del mes para desbloquear los bonos.</p>
               </div>
            </div>
          )}
          ...
          <div className="bg-white/5 border border-white/10 rounded-[3rem] p-10 flex flex-col md:flex-row items-center justify-between gap-10 shadow-2xl">
             <div className="space-y-2 text-center md:text-left">
                ...
                <p className="text-slate-500 text-sm font-medium">Cierre: <span className="text-primary font-black">{formattedClosingDate}</span></p>
                <div className="flex items-center gap-2 mt-4 justify-center md:justify-start">
                   <span className="material-symbols-outlined text-rose-500 text-sm">schedule</span>
                   <span className="text-xs font-black text-slate-400 uppercase tracking-widest">{daysRemaining} días para el corte</span>
                </div>
             </div>
             
             <div className="flex flex-col sm:flex-row items-center gap-8 md:gap-16">
                <div className="text-center">
                   <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Bonos Acumulados</p>
                ...
                <div className="text-center">
                   <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Estatus de Periodo</p>
                   <div className={`px-6 py-2.5 rounded-2xl border text-[10px] font-black uppercase tracking-widest ${
                      ...
                      {allTasksDone ? 'CALIFICADO' : 'EN OPERACIÓN'}
                   </div>
                </div>
             </div>
          </div>
