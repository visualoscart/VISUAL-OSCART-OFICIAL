...
const Team: React.FC = () => {
  ...
  const openTaskInCalendar = (taskId: string) => {
    navigate('/calendar', { state: { openTaskId: taskId } });
  };
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {myPendingTasks.length > 0 ? myPendingTasks.map(t => {
                  ...
                  return (
                    <div key={t.id} className="bg-card-dark border border-white/5 p-6 rounded-[2rem] flex items-center justify-between group hover:border-primary/30 transition-all animate-in fade-in cursor-pointer" onClick={() => openTaskInCalendar(t.id)}>
                      ...
                      <div className="flex gap-2">
                        <button 
                          onClick={(e) => { e.stopPropagation(); openTaskInCalendar(t.id); }}
                          className="w-10 h-10 rounded-xl bg-white/5 text-slate-400 hover:text-white transition-all flex items-center justify-center"
                          title="Abrir Tarea"
                        >
