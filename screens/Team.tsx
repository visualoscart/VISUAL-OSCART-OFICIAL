...
const Team: React.FC = () => {
  ...
  const receiptDetails = useMemo(() => {
    ...
    // Si ya tiene los datos, usarlos
    if (selectedReceipt.completedTasks !== undefined && selectedReceipt.completedTasks !== null) {
      return {
        completedTasks: selectedReceipt.completedTasks,
        tasksTotal: selectedReceipt.tasksTotal || 0
      };
    }

    // Fallback: Calcular en tiempo real basado en las tareas del periodo
    const monthNames = ["ENERO", "FEBRERO", "MARZO", "ABRIL", "MAYO", "JUNIO", "JULIO", "AGOSTO", "SEPTIEMBRE", "OCTUBRE", "NOVIEMBRE", "DICIEMBRE"];
    const monthIndex = monthNames.indexOf(selectedReceipt.month.toUpperCase());
    
    if (monthIndex === -1) return { completedTasks: 0, tasksTotal: 0 };

    const periodTasks = (tasks || []).filter(t => {
      const d = new Date(t.date);
      return d.getMonth() === monthIndex && 
             d.getFullYear() === selectedReceipt.year && 
    ...
    return {
      ...
      tasksTotal: Math.max(0, calculatedTasksTotal)
    };
