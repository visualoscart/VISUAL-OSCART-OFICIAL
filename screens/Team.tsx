...
const Team: React.FC = () => {
  ...
  const upcomingBirthdays = useMemo(() => {
    ...
    return usersDB.filter(u => u.birthDate).map(u => { const bd = new Date(u.birthDate + 'T12:00:00'); return { ...u, bdDay: bd.getDate(), bdMonth: bd.getMonth() }; })
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 pb-20">
          ...
          <div className="lg:col-span-8 space-y-12">
            <section className="space-y-6">
              <h3 className="text-xl font-display text-white uppercase italic flex items-center gap-3"><span className="material-symbols-outlined text-primary">assignment</span>Pendientes Operativos</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl overflow-hidden bg-white/5 flex items-center justify-center shrink-0 border border-white/5 shadow-sm">
                           {p?.logoUrl ? (
                             ...
                             <span className="material-symbols-outlined text-slate-600 text-sm">business</span>
                           )}
                        </div>
                        <div>
                ...
                }) : (<div className="col-span-full py-16 text-center border border-dashed border-white/5 rounded-[2.5rem] text-slate-600 font-bold text-[10px] uppercase tracking-widest italic">Todo al día.</div>)}
              </div>
            </section>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-background-dark/95 backdrop-blur-3xl animate-in fade-in" onClick={() => setSelectedReceipt(null)}>
          <div className="max-w-xl w-full flex flex-col items-center gap-6" onClick={e => e.stopPropagation()}>
            <div ref={receiptPrintRef} className="w-full bg-white text-slate-900 rounded-[2.5rem] shadow-2xl p-12 space-y-10">
                ...
                <p className="text-[8px] text-center text-slate-300 font-bold uppercase tracking-widest italic pt-4">Visual Oscart Network • Liquidación Algorítmica Verificada</p>
            </div>
