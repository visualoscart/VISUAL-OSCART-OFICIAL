                  ...
                  {dayTasks.map(t => { 
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* El título solo se muestra en escritorio/tablets */}
      ...
      {selectedTaskDetail && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-background-dark/95 backdrop-blur-xl animate-in fade-in" onClick={() => { if(!isEditingDetail) setSelectedTaskDetail(null); }}>
          <div className="max-w-2xl w-full bg-card-dark border border-white/10 rounded-[2rem] sm:rounded-[3rem] shadow-2xl p-6 sm:p-10 flex flex-col max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
             ...
             <div className="flex-1 space-y-6 sm:space-y-8 pr-1">
                ...
                <div className="space-y-2">
                  ...
                  {isEditingDetail ? (
                    ...
                    <div className="bg-white/5 p-4 sm:p-6 rounded-2xl text-slate-300 text-sm italic border border-white/5">"{selectedTaskDetail.description || 'Sin instrucciones.'}"</div>
                  )}
                </div>
             ...
             <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-8 sm:mt-10 shrink-0">
               {isEditingDetail ? (
                 <>
                   <button onClick={() => setIsEditingDetail(false)} className="py-4 bg-white/5 text-slate-500 font-black text-[9px] sm:text-[10px] uppercase rounded-2xl">Cancelar</button>
                   <button onClick={handleSaveEdit} disabled={isSubmitting} className="py-4 bg-primary text-white font-black text-[9px] sm:text-[10px] uppercase rounded-2xl shadow-xl">{isSubmitting ? 'Guardando...' : 'Guardar Cambios'}</button>
                 </>
               ) : (
                 <>
                   <button onClick={handleDeleteClick} disabled={isSubmitting} className={`py-4 font-black text-[9px] sm:text-[10px] uppercase rounded-2xl border transition-all ${confirmDeleteStep ? 'bg-rose-600 text-white border-rose-400 animate-pulse' : 'bg-rose-500/10 text-rose-500 border-rose-500/20 hover:bg-rose-500 hover:text-white'}`}>
                     {isSubmitting ? 'Borrando...' : confirmDeleteStep ? '¿ESTÁS SEGURO?' : 'Eliminar'}
                   </button>
                   <button onClick={async () => { await toggleTaskStatus(selectedTaskDetail.id); setSelectedTaskDetail(null); }} className={`py-4 font-black text-[9px] sm:text-[10px] uppercase rounded-2xl shadow-xl transition-all active:scale-95 ${selectedTaskDetail.status === 'Completada' ? 'bg-slate-700' : 'bg-emerald-500 text-white'}`}>
