...
const Calendar: React.FC = () => {
  ...
  const handleDuplicate = () => {
    ...
    setTaskForm({
      ...
      contentType: selectedTaskDetail.contentType || 'Post'
    });
    setSelectedTaskDetail(null);
    setShowModal(true);
  };
          ...
          <div className="max-w-xl w-full bg-card-dark border border-white/10 rounded-[2rem] sm:rounded-[3rem] shadow-2xl p-6 sm:p-10 space-y-6 overflow-y-auto max-h-[90vh]" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSaveTask} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-500 uppercase px-1">Tipo de Contenido</label>
                    <select required className="w-full bg-background-dark border border-white/10 rounded-xl p-3 text-white text-xs outline-none focus:border-primary" value={taskForm.contentType} onChange={e => setTaskForm({...taskForm, contentType: e.target.value as any})}>
          ...
          <div className="max-w-2xl w-full bg-card-dark border border-white/10 rounded-[2rem] sm:rounded-[3rem] shadow-2xl p-6 sm:p-10 flex flex-col max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
             <div className="flex justify-between items-start mb-6 sm:mb-8 shrink-0">
                <div className="flex items-center gap-3 sm:gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[8px] sm:text-[10px] text-primary font-black uppercase tracking-widest mt-1">
                      {selectedTaskDetail.date} • {selectedTaskDetail.status}
                    </p>
                  </div>
                </div>
             ...
             <div className="flex-1 space-y-6 sm:space-y-8 pr-1">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="flex items-center gap-3 sm:gap-4 p-4 bg-white/5 rounded-2xl border border-white/5 min-w-0">
                    {projects.find(p => p.id === selectedTaskDetail.projectId)?.logoUrl && <img src={projects.find(p => p.id === selectedTaskDetail.projectId)?.logoUrl} className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl object-cover shrink-0" />}
                    <div className="min-w-0 flex-1">
                      <p className="text-[8px] sm:text-[9px] font-black text-slate-500 uppercase tracking-widest">Marca</p>
                      <p className="text-xs sm:text-sm font-black text-white truncate">{projects.find(p => p.id === selectedTaskDetail.projectId)?.name}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 sm:gap-4 p-4 bg-white/5 rounded-2xl border border-white/5 min-w-0">
                    {usersDB.find(u => u.id === selectedTaskDetail.collaboratorId)?.avatar && <img src={usersDB.find(u => u.id === selectedTaskDetail.collaboratorId)?.avatar} className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl object-cover shrink-0" />}
                    <div className="min-w-0 flex-1">
                      <p className="text-[8px] sm:text-[9px] font-black text-slate-500 uppercase tracking-widest">Responsable</p>
                      <p className="text-xs sm:text-sm font-black text-white truncate">{usersDB.find(u => u.id === selectedTaskDetail.collaboratorId)?.firstName}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 sm:gap-4 p-4 bg-primary/10 rounded-2xl border border-primary/10 min-w-0">
                    <span className="material-symbols-outlined text-primary text-xl shrink-0">category</span>
                    <div className="min-w-0 flex-1">
                      <p className="text-[8px] sm:text-[9px] font-black text-primary uppercase tracking-widest">Contenido</p>
                      <p className="text-xs sm:text-sm font-black text-white truncate">{selectedTaskDetail.contentType || 'Post'}</p>
                    </div>
                  </div>
                </div>
                ...
                {isEditingDetail && (
                    <div className="space-y-1">
                        <label className="text-[9px] font-black text-slate-500 uppercase px-1">Tipo de Contenido</label>
                        <select className="w-full bg-background-dark border border-white/10 rounded-xl p-3 text-white text-xs outline-none focus:border-primary" value={editForm.contentType || 'Post'} onChange={e => setEditForm({...editForm, contentType: e.target.value as any})}>
                            <option value="Post">Post</option>
                            <option value="Video">Video</option>
                            <option value="Historia">Historia</option>
                            <option value="Carrusel">Carrusel</option>
