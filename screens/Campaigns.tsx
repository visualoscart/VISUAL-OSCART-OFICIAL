      ...
      if (pdfElement) {
        ...
        try {
          ...
          if (summaryEl) {
            ...
            pdf.addImage(imgData, 'JPEG', 0, 0, pageWidth, imgHeight > pageHeight ? pageHeight : imgHeight);
          }
          ...
          for (let i = 1; i <= totalPages; i++) {
            ...
            if (logoData) {
              try {
                ...
                pdf.addImage(logoData, 'PNG', 11, pageHeight - 14, 8, 8);
              } catch (e) {
        ...
        <div ref={pdfRef} id="campaign-pdf" className="p-0 bg-white text-slate-900 font-sans relative w-[794px]">
          {selectedCampaign && (
            <div className="w-full">
              ...
              <div id="pdf-summary-page" className="p-10 space-y-12 bg-white min-h-[1050px] flex flex-col">
                <div className="grow space-y-12">
                  {/* LOGO & BRAND */}
                  <div className="flex items-center justify-between mb-8 border-b-2 border-slate-900 pb-8">
                    <div className="flex items-center gap-6">
                      <div className="w-20 h-20 bg-slate-100 p-0 rounded-2xl flex items-center justify-center overflow-hidden">
                  ...
                  <div className="space-y-6">
                     ...
                     <div className="grid grid-cols-1 gap-4">
                        {selectedCampaign.themes.map((theme, i) => (
                          <div key={theme.id} className="p-6 border border-slate-100 bg-slate-50/50 rounded-2xl flex justify-between items-center group">
                            <div className="flex gap-4 items-center">
                               <div className="w-8 h-8 rounded-lg bg-slate-900 text-white flex items-center justify-center font-bold text-sm">
                                 {i + 1}
                               </div>
                               <div className="flex flex-col">
                                  <span className="text-[14px] font-black text-slate-900 uppercase">{theme.title}</span>
                                  <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">{theme.format}</span>
                               </div>
                            </div>
                            <div className="text-right">
                               <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">PRODUCCIÓN</div>
                               <div className="text-xs font-black text-slate-800">
                                 {selectedCampaign.productionDates.find(d => d.id === theme.productionId)?.date || 'Pendiente'}
                               </div>
                            </div>
                          </div>
                        ))}
                ...
                {/* Space for the programmatic footer */}
                <div className="h-20"></div>
              </div>

              {/* DETAILED SCRIPTS: EACH ON NEW PAGE */}
              {selectedCampaign.themes.map((theme, i) => (
                <div key={theme.id} className="pdf-theme-page p-10 pt-16 bg-white min-h-[1050px] flex flex-col">
                   <div className="grow space-y-8">
                     <div className="flex items-center justify-between border-b border-slate-900 pb-4">
                        <h3 className="text-2xl font-black uppercase text-slate-900 leading-tight">
                          Pilar {i + 1}: {theme.title}
                        </h3>
                        <div className="px-5 py-2 bg-slate-100 text-[11px] font-black text-slate-600 rounded-full tracking-widest">
                          FORMATO: {theme.format}
                        </div>
                     </div>
                     <div className="bg-slate-50 p-12 rounded-[3rem] border border-slate-100/50 shadow-sm relative">
