...
const Campaigns: React.FC = () => {
 ...
 const [selectedThemeForTask, setSelectedThemeForTask] = useState<CampaignTheme | null>(null);
 ...
 const handleOpenTaskModal = (theme: CampaignTheme) => {
 if (!editingCampaignId) {
 showToast("Guarda la campaña antes de agendar misiones","error");
 return;
 }
 if (!theme.title) {
 showToast("El pilar debe tener un título","error");
 return;
 }
 setSelectedThemeForTask(theme);
 setTaskForm({
 title: theme.title,
 ...
 showToast("Misión agendada en calendario","success");
 };
 ...
 const code = (brand?.typography?.brandCode || brand?.name || 'MARCA').toUpperCase();
 ...
 const usableHeight = pageHeight - 25; // More room for footer safety
 const splitThreshold = 25; // Match buffer

 // Wait for fonts and styles to stabilize
 ...
 // 3. Capture Calendar Page (ATOMIC ROW-BY-ROW RENDERING)
 ...
 // Check if it fits (add small buffer)
 if (currentY + rowH > pageLimit) {
 pdf.addPage();
 currentY = margin;
 // Repeat Header on new page for context
 pdf.addImage(headerCanvas.toDataURL('image/jpeg', 0.95), 'JPEG', margin, currentY, headerW, headerH);
 currentY += headerH + 5;
 }
 ...
 // Footer Text (Centered with logo)
 pdf.setFontSize(7);
 pdf.setTextColor(120, 120, 120);
 pdf.setFont(undefined, 'normal');
 const footerLabel = `Documento Confidencial - Estrategia de Campaña ${campaign.month} ${campaign.year} - ${brand?.name?.toUpperCase() || 'MARCA'}`;
 ...
 <div className="absolute -top-24 -right-24 w-[500px] h-[500px] bg-primary/5 blur-[120px] rounded-full pointer-events-none"></div>
 <div className="absolute top-1/2 -left-24 w-[350px] h-[350px] bg-primary/5 blur-[100px] rounded-full pointer-events-none"></div>
 ...
 className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-white/5 border border-white/5 border-t-white/15/10 text-white font-black text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all shadow-xl"
 >
 ...
 className="group relative flex flex-col p-8 rounded-[2.5rem] bg-card-dark border border-white/5 border-t-white/15/5 hover:border-primary/30 transition-all duration-500 overflow-hidden text-left shadow-2xl"
 >
 ...
 <div className="w-20 h-20 rounded-2xl bg-[#2a2a2a] p-0 flex items-center justify-center overflow-hidden shrink-0 border border-white/5 border-t-white/15/10 shadow-inner">
 {project.logoUrl ? (
 ...
 <div>
 <h3 className="text-xl font-black text-white tracking-tight grow line-clamp-1 uppercase">{project.name}</h3>
 ...
 <p className="text-[10px] font-black uppercase tracking-widest text-slate-300">Estrategias Guardadas</p>
 ...
 {/* BRAND NAV & ACTIONS */}
 <div className="flex items-center justify-between p-6 rounded-[2.5rem] bg-white/5 border border-white/5 border-t-white/15/10 backdrop-blur-xl shadow-2xl">
 <div className="flex items-center gap-4">
 <div className="w-16 h-16 rounded-xl bg-[#2a2a2a] p-0 flex items-center justify-center overflow-hidden border border-white/5 border-t-white/15/10 shadow-lg">
 <img src={selectedProject.logoUrl} className="w-full h-full object-cover"alt=""referrerPolicy="no-referrer"/>
 </div>
 <h2 className="text-2xl font-black text-white tracking-tighter uppercase">{selectedProject.name}</h2>
 </div>
 
 <div className="flex gap-4">
 <button 
 onClick={() => { setIsViewBodega(true); setIsCreating(false); }}
 className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${isViewBodega && !isCreating ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-white/5 text-slate-500 hover:text-white border border-white/5 border-t-white/15/5'}`}
 >
 ...
 className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${isCreating ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-white/5 text-slate-500 hover:text-white border border-white/5 border-t-white/15/5'}`}
 >
 ...
 <div className="flex flex-col items-center justify-center py-20 bg-white/5 border border-dashed border-white/5 border-t-white/15 rounded-[2.5rem] text-center space-y-6">
 ...
 <p className="text-slate-500 max-w-xs mx-auto mt-2">Aún no has creado estrategias para {selectedProject.name}. ¡Empieza ahora!</p>
 </div>
 <button 
 onClick={() => { setIsCreating(true); setIsViewBodega(false); }}
 ...
 <div key={camp.id} className="group bg-card-dark border border-white/5 border-t-white/15/5 p-6 rounded-[2rem] hover:border-white/20 transition-all flex flex-col gap-6">
 ...
 <div className="flex flex-col items-center p-3 rounded-2xl bg-white/5 border border-white/5 border-t-white/15/5">
 ...
 className="w-full py-4 bg-white/5 hover:bg-white/10 border border-white/5 border-t-white/15/5 rounded-2xl text-white font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 transition-all"
 >
 ...
 <div className="p-8 rounded-[2rem] bg-card-dark border border-white/5 border-t-white/15/5 space-y-8 sticky top-8">
 <div className="space-y-4">
 <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Mes de Campaña</label>
 <select 
 value={month}
 onChange={(e) => setMonth(e.target.value)}
 className="w-full p-4 rounded-2xl bg-white/5 border border-white/5 border-t-white/15/10 text-white font-bold outline-none ring-0 focus:border-primary transition-all"
 >
 ...
 className="w-full p-4 rounded-2xl bg-white/5 border border-white/5 border-t-white/15/10 text-white font-bold"
 />
 </div>
 ...
 className="w-full h-32 p-4 rounded-2xl bg-white/5 border border-white/5 border-t-white/15/10 text-white text-sm resize-none focus:border-primary transition-all outline-none"
 />
 </div>
 ...
 className="w-full p-4 rounded-xl bg-white/5 border border-white/5 border-t-white/15/10 text-white font-bold"
 />
 </div>
 <div className="space-y-2">
 ...
 : 'bg-white/5 border-white/5 border-t-white/15 text-slate-500 hover:text-white hover:bg-white/10'
 }`}
 >
 ...
 <button 
 onClick={() => handleOpenTaskModal(theme)}
 className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all text-[9.5px] font-black uppercase tracking-wider ${
 tasks.some(t => t.campaignThemeId === theme.id)
 ? 'bg-primary/20 border-primary text-primary' 
 : 'bg-primary/10 border-primary/20 text-primary hover:bg-primary hover:text-white'
 }`}
 >
 <span className="material-symbols-outlined text-sm">
 {tasks.some(t => t.campaignThemeId === theme.id) ? 'event_available' : 'calendar_add_on'}
 </span>
 ...
 className="w-full h-64 p-6 rounded-3xl bg-white/5 border border-white/5 border-t-white/15/10 text-white text-sm resize-none scrollbar-hide focus:border-primary/50 transition-all font-sans"
 />
 <p className="mt-2 text-[9px] text-slate-500">El contenido se guardará automáticamente dentro de este tema.</p>
 </div>
 )}
 </div>
 </div>
 ...
 <div ref={pdfRef} id="campaign-pdf" className="p-0 bg-white text-slate-900 font-sans relative w-[1200px]">
 {selectedCampaign && (
 ...
 <div className="space-y-4">
 <h2 className="text-sm font-black uppercase tracking-[0.3em] text-slate-500 border-b border-white/5 border-t-white/15 pb-2">Objetivo de la Campaña</h2>
 ...
 <div className="space-y-1">
 <span className="text-xs font-black uppercase tracking-[0.4em]" style={{ color: projects.find(p => p.id === selectedCampaign.projectId)?.brandColors?.[0] || '#8c2bee' }}>Pilar {i + 1}</span>
 ...
 <div id="pdf-calendar-page" className="p-10 pb-24 pt-16 bg-white flex flex-col min-h-[1100px]">
 ...
 .map((task) => (
 <div key={task.id} className="pdf-calendar-row grid grid-cols-12 gap-4 px-6 py-5 border border-white/5 border-t-white/15 bg-gradient-to-br from-[#363636] to-[#2a2a2a] shadow-xl shadow-black/40 rounded-[2rem] items-center shadow-[0_8px_30px_rgba(0,0,0,0.3)] relative overflow-hidden">
 <div className="absolute left-0 top-0 bottom-0 w-1" style={{ backgroundColor: projects.find(p => p.id === selectedCampaign.projectId)?.brandColors?.[0] || '#8c2bee' }}></div>
 ...
 {selectedCampaign.themes.find(th => th.id === task.campaignThemeId)?.format || 'Video'}
 </span>
 </div>
 </div>
 ))}

 {tasks.filter(t => t.campaignId === selectedCampaign.id).length === 0 && (
 ...
 <div className="mt-20 pt-10 border-t border-white/5 border-t-white/15 text-[11px] text-slate-500 text-center">
 ...
 {showTaskModal && (
 <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6 bg-background-dark/95 backdrop-blur-2xl animate-in fade-in"onClick={() => setShowTaskModal(false)}>
 <div className="max-w-xl w-full glass-panel border border-white/5 border-t-white/15/10 rounded-[2rem] sm:rounded-[3rem] shadow-2xl p-6 sm:p-12 space-y-8 overflow-y-auto max-h-[90vh]"onClick={e => e.stopPropagation()}>
 <div className="flex items-center justify-between">
 <h3 className="text-xl sm:text-2xl font-black text-white uppercase tracking-tighter">Agendar <span className="text-primary">Misión</span></h3>
 <button 
 onClick={() => setShowTaskModal(false)}
 className="w-10 h-10 rounded-xl bg-[#202020]/5 flex items-center justify-center text-slate-500 hover:text-white transition-all"
 >
 ...
 <p className="text-[9px] font-black text-primary uppercase tracking-widest">Vinculando a Pilar:</p>
 <p className="text-sm font-bold text-white truncate">{selectedThemeForTask?.title}</p>
 </div>

 <form onSubmit={handleScheduleTask} className="space-y-6">
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
 <div className="space-y-2">
 <label className="text-[9px] font-black text-slate-500 uppercase px-1 tracking-widest">Marca Estratégica</label>
 <div className="w-full bg-black/60 border border-white/5 border-t-white/15/10 rounded-2xl p-4 text-slate-500 text-xs font-black uppercase">
 ...
 <input required className="w-full bg-black/40 border border-white/5 border-t-white/15/10 rounded-2xl p-5 text-white text-sm font-bold uppercase outline-none focus:border-primary"value={taskForm.title} onChange={e => setTaskForm({...taskForm, title: e.target.value})} placeholder="Título de la misión..."/>
 </div>
 <div className="space-y-2">
 ...
 <input type="date"required className="w-full bg-black/40 border border-white/5 border-t-white/15/10 rounded-2xl p-4 text-white text-xs outline-none focus:border-primary"value={taskForm.date} onChange={e => setTaskForm({...taskForm, date: e.target.value})} />
 </div>
 <div className="space-y-2">
 <label className="text-[9px] font-black text-slate-500 uppercase px-1 tracking-widest">Punto de Acceso (Opcional)</label>
 <input className="w-full bg-black/40 border border-white/5 border-t-white/15/10 rounded-2xl p-4 text-white text-xs outline-none focus:border-primary"value={taskForm.driveLink} onChange={e => setTaskForm({...taskForm, driveLink: e.target.value})} placeholder="Link de referencia..."/>
 </div>
 </div>
 <div className="flex gap-4 pt-4">
 <button type="button"onClick={() => setShowTaskModal(false)} className="flex-1 py-5 bg-white/5 text-slate-500 font-black text-[10px] uppercase rounded-[2rem] hover:text-white transition-colors">Cancelar</button>
 <button type="submit"disabled={isSubmittingTask} className="btn-premium flex-[2] py-5 text-white font-black text-[11px] uppercase rounded-[2rem] shadow-2xl">{isSubmittingTask ? 'Procesando...' : 'Confirmar Misión'}</button>
 </div>
 </form>
 </div>
 </div>
