      ...
      <header className="px-8 py-6 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 bg-background-dark/30 backdrop-blur-2xl shrink-0 z-20">
        ...
        {selectedProject && (
          <button 
            ...
            className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-white/5 border border-white/10 text-white font-black text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all shadow-xl font-black"
          >
                      ...
                      {tasks
                        ...
                        <div key={task.id} className="pdf-calendar-row grid grid-cols-12 gap-4 px-6 py-5 border border-slate-100 bg-slate-50/30 rounded-2xl items-center">
                           <div className="col-span-3 flex flex-col">
                              <span className="text-[12px] font-black text-slate-900 italic">{format(new Date(task.date + 'T12:00:00'), 'dd MMM', { locale: es }).toUpperCase()}</span>
                              <span className="text-[9px] font-bold text-slate-400 uppercase">{format(new Date(task.date + 'T12:00:00'), 'eeee', { locale: es })}</span>
                           </div>
                           <div className="col-span-6">
                              <h4 className="text-[14px] font-black text-slate-900 uppercase leading-none mb-1">{task.title}</h4>
                              <p className="text-[9px] text-slate-500 line-clamp-1 italic">{selectedCampaign.themes.find(th => th.id === task.campaignThemeId)?.title || 'Estrategia'}</p>
                           </div>
                           <div className="col-span-3 text-right">
                              <span className="px-4 py-1.5 bg-white border border-slate-200 rounded-full text-[9px] font-black text-slate-600 uppercase tracking-tighter shadow-sm">
