    ...
    setTimeout(async () => {
      ...
      if (pdfElement) {
        ...
        try {
          ...
          const calendarHeader = document.querySelector('#pdf-calendar-page .grid.bg-slate-900');
            ...
            <div className="w-full">
                        ...
                        {selectedCampaign.themes.map((theme, i) => (
                          <div key={theme.id} className="p-3 border border-slate-100 bg-slate-50/50 rounded-xl flex justify-between items-center group">
                            <div className="flex gap-3 items-center">
                               ...
                               <div className="flex flex-col">
                                  <span className="text-[16px] font-black text-slate-900 uppercase leading-snug">{theme.title}</span>
                                  <span className="text-[12px] font-bold text-slate-500 uppercase tracking-widest">{theme.format}</span>
                               </div>
                            </div>
                            <div className="text-right">
                               <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">PROD.</div>
                ...
                <div key={theme.id} className="pdf-theme-page p-10 pb-24 pt-16 bg-white flex flex-col">
                    <div className="grow space-y-8">
                      <div className="flex items-center justify-between border-b-2 border-slate-900 pb-3 mb-2">
                         ...
                         <div className="flex items-center justify-center px-6 py-2 bg-slate-900 text-[14px] font-black text-white rounded-full tracking-[0.2em] leading-none">
                           FORMATO: {theme.format}
                         </div>
                      </div>
                      <div className="relative pt-1">
                         <pre className="whitespace-pre-wrap font-sans text-lg leading-relaxed text-slate-900 tracking-tight">
              ...
              <div id="pdf-calendar-page" className="p-10 pb-24 pt-16 bg-white flex flex-col min-h-[1100px]">
                <div className="grow space-y-10">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-12 gap-4 px-6 py-4 bg-slate-900 rounded-2xl text-white font-black text-[10px] uppercase tracking-widest">
                    ...
                    <div className="space-y-3">
                      {tasks
                        ...
                        .map((task) => (
                        <div key={task.id} className="pdf-calendar-row grid grid-cols-12 gap-4 px-6 py-5 border border-slate-100 bg-slate-50/30 rounded-2xl items-center">
                           <div className="col-span-3 flex flex-col">
                              <span className="text-[16px] font-black text-slate-900 italic">{format(new Date(task.date + 'T12:00:00'), 'dd MMM', { locale: es }).toUpperCase()}</span>
                              <span className="text-[12px] font-bold text-slate-400 uppercase">{format(new Date(task.date + 'T12:00:00'), 'eeee', { locale: es })}</span>
                           </div>
                           <div className="col-span-6">
                              <h4 className="text-[19px] font-black text-slate-900 uppercase leading-none mb-1">{task.title}</h4>
                              <p className="text-[14px] text-slate-500 line-clamp-1 italic">{selectedCampaign.themes.find(th => th.id === task.campaignThemeId)?.title || 'Estrategia'}</p>
                           </div>
                           <div className="col-span-3 text-right">
                              <span className="px-4 py-1.5 bg-white border border-slate-200 rounded-full text-[14px] font-black text-slate-600 uppercase tracking-tighter shadow-sm">
                      ...
                      {tasks.filter(t => t.campaignId === selectedCampaign.id).length === 0 && (
                        <div className="py-20 text-center space-y-4 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
                           ...
                           <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">No hay misiones agendadas para generar el calendario</p>
                        </div>
                      )}
                    </div>
                  </div>

                  <div className="mt-20 pt-10 border-t border-slate-100 italic text-[11px] text-slate-400 text-center">
            ...
            <form onSubmit={handleScheduleTask} className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <div className="w-full bg-black/60 border border-white/10 rounded-2xl p-4 text-slate-400 text-xs font-black uppercase">
              ...
              <div className="space-y-2">
                ...
                <input required className="w-full bg-black/40 border border-white/10 rounded-2xl p-5 text-white text-sm font-bold uppercase outline-none focus:border-primary" value={taskForm.title} onChange={e => setTaskForm({...taskForm, title: e.target.value})} placeholder="Título de la misión..." />
              </div>
              <div className="space-y-2">
                <label className="text-[9px] font-black text-slate-500 uppercase px-1 tracking-widest">Protocolo de Instrucción</label>
                <textarea className="w-full bg-black/40 border border-white/10 rounded-2xl p-5 text-white text-sm h-32 outline-none focus:border-primary resize-none italic leading-relaxed" value={taskForm.description} onChange={e => setTaskForm({...taskForm, description: e.target.value})} placeholder="Instrucciones detalladas..." />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <input className="w-full bg-black/40 border border-white/10 rounded-2xl p-4 text-white text-xs outline-none focus:border-primary italic" value={taskForm.driveLink} onChange={e => setTaskForm({...taskForm, driveLink: e.target.value})} placeholder="Link de referencia..." />
                </div>
              </div>
