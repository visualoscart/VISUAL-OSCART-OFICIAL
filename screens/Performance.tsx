...
const Performance: React.FC = () => {
  ...
  const operationalLogs = useMemo(() => {
    ...
    // Get tasks from this month that are completed/published
    return tasks.filter(t => {
      ...
      const matchCreatedAt = cDate && (MONTHS[cDate.getMonth()] === selectedMonth && cDate.getFullYear() === selectedYear);
      const matchTargetDate = tDate && (MONTHS[tDate.getMonth()] === selectedMonth && tDate.getFullYear() === selectedYear);
      
      return matchCreatedAt || matchTargetDate;
    }).sort((a,b) => {
      const dateA = new Date(a.date || a.createdAt).getTime();
      const dateB = new Date(b.date || b.createdAt).getTime();
      return dateA - dateB;
    });
  }, [selectedProjectId, selectedMonth, selectedYear, tasks]);
                               ...
                               {operationalLogs.length > 0 ? operationalLogs.slice(0, 10).map((log, i) => (
                                 <div key={i} className="flex flex-col p-3 rounded-xl bg-white/5 border border-white/5 gap-1">
                                   <div className="flex justify-between items-center">
                                      ...
                                      <span className="text-[8px] font-mono text-slate-500 italic">{format(new Date(log.createdAt), 'dd MMM')}</span>
                                   </div>
                                 </div>
                               )) : (
          ...
          {selectedProject && (() => {
            ...
            return (
              <div className="bg-white font-sans text-slate-900">
                ...
                <div id="pdf-page-1" className="p-20 min-h-[1754px] bg-white relative flex flex-col border-[20px] border-slate-50">
                    <div className="flex items-center gap-10 border-b border-slate-200 pb-12 mb-16">
                       ...
                       <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-4 mt-2">
                             ...
                             <span className="text-[14px] font-black text-slate-400 uppercase tracking-widest">{selectedProject?.name}</span>
                          </div>
                       </div>
                    </div>
                    ...
                    {executiveSummary && (
                       <div className="mt-12 space-y-6">
                         <div className="bg-[#dbdbdb] p-10 border border-slate-300 rounded-3xl relative overflow-hidden">
                            ...
                            <p className="text-slate-700 text-3xl leading-relaxed font-black italic tracking-tighter whitespace-pre-wrap relative z-10">{executiveSummary}</p>
                         </div>
                       </div>
                    )}
                </div>
                ...
                <div id="pdf-page-2" className="p-20 min-h-[1754px] bg-white relative space-y-12 flex flex-col border-[20px] border-slate-50" style={{ pageBreakBefore: 'always' }}>
                              ...
                              return (
                                <tr key={platform} className="even:bg-slate-50 hover:bg-slate-100 transition-colors">
                                  ...
                                  <td className="p-6 text-right">
                                    <p className="text-lg font-black text-slate-900">+{metrics[platform]?.followers?.toLocaleString() || '0'}</p>
                                    {fComp && (
                                       <p className={`text-[11px] font-black uppercase ${fComp.increased ? 'text-emerald-600' : 'text-rose-600'}`}>
                                          {fComp.increased ? '↑' : '↓'} {fComp.percent}% {MONTHS[(MONTHS.indexOf(selectedMonth) - 1 + 12) % 12]}
                                       </p>
                                    )}
                                  </td>
                                  <td className="p-6 text-right font-black text-slate-700 text-lg">{metrics[platform]?.totalFollowers?.toLocaleString() || '-'}</td>
                                  <td className="p-6 text-right">
                                    <p className="font-black text-slate-700 text-lg">{metrics[platform]?.reach?.toLocaleString() || '0'}</p>
                                    {rComp && (
                                       <p className={`text-[11px] font-black uppercase ${rComp.increased ? 'text-emerald-600' : 'text-rose-600'}`}>
                ...
                <div id="pdf-page-3" className="p-20 min-h-[1754px] bg-white relative space-y-20 flex flex-col border-[20px] border-slate-50" style={{ pageBreakBefore: 'always' }}>
                   <div className="space-y-10">
                      <div className="flex items-center justify-between border-b border-slate-200 pb-6">
                                      ...
                                      {operationalLogs.length > 0 ? operationalLogs.slice(0, 30).map((task, i) => (
                                          <tr key={i} className={`text-[11px] font-bold text-slate-800 ${i % 2 !== 0 ? 'bg-slate-50' : 'bg-white'}`}>
                                              <td className="p-4 border-r border-slate-100 text-slate-600 font-mono italic">
                                                {safeFormat(task.date || task.createdAt, 'dd/MM/yyyy')}
                                              </td>
