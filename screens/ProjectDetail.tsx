...
const ProjectDetail: React.FC = () => {
  ...
  const { projects, usersDB, updateProject, addMediaAsset, deleteMediaAsset, addTextAsset, deleteTextAsset } = useProjects();
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {(project.textRepository || []).map(item => (
                      <div key={item.id} className="bg-card-dark p-8 rounded-[2.5rem] border border-white/5 relative group hover:border-accent-orange/30 shadow-xl transition-all">
                        <button onClick={() => deleteTextAsset(project.id, item.id)} className="absolute top-6 right-6 text-rose-500/30 hover:text-rose-500 transition-colors"><span className="material-symbols-outlined">delete</span></button>
